#!/usr/bin/env python3
"""Build an FTS5 index over readmes.name + readme for free-text search.

Creates `readmes_fts` as an external-content FTS5 table backed by the
`readmes` table. Only rows with `status = 'fetched'` are indexed (missing
entries have no readme text). Because `repo` is a TEXT primary key (not an
INTEGER alias for the rowid), `content_rowid` references the implicit SQLite
rowid. Tokenizer is `porter unicode61` for stemming + diacritic folding.

Re-runnable: drops the virtual table and rebuilds from scratch. Run after
every refresh of readmes.db.
"""

import sqlite3
import sys
import time
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parent.parent.parent
DB_PATH = REPO_ROOT / "data" / "github" / "readmes.db"


def main() -> int:
    if not DB_PATH.is_file():
        print(f"missing: {DB_PATH}", file=sys.stderr)
        return 1

    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()

    t0 = time.time()
    cur.execute("DROP TABLE IF EXISTS readmes_fts")
    cur.execute("""
        CREATE VIRTUAL TABLE readmes_fts USING fts5(
            name,
            readme,
            content='readmes',
            content_rowid='rowid',
            tokenize='porter unicode61'
        )
    """)
    cur.execute(
        "INSERT INTO readmes_fts(rowid, name, readme) "
        "SELECT rowid, name, readme FROM readmes "
        "WHERE status = 'fetched' AND readme IS NOT NULL"
    )
    con.commit()

    indexed = cur.execute("SELECT COUNT(*) FROM readmes_fts").fetchone()[0]
    db_size = DB_PATH.stat().st_size

    con.close()
    elapsed = time.time() - t0
    print(
        f"Done in {elapsed:.1f}s. "
        f"Indexed {indexed} READMEs. "
        f"DB file is now {db_size / (1024**2):.1f} MB."
    )
    return 0


if __name__ == "__main__":
    sys.exit(main())
