#!/usr/bin/env python3
"""Download and SHA-1-verify the latest simplewiki multistream dump.

Fetches both the article bz2 and its index from
``https://dumps.wikimedia.org/simplewiki/latest/`` into
``data/simplewiki/dumps/``. Each file is streamed to a ``.tmp`` sibling and
atomically renamed only after the SHA-1 matches Wikimedia's manifest, so an
interrupted transfer can never leave a partial file under the canonical
name. Re-running with files already in place skips them if they verify.

Source-specific (no enwiki). Ported from the local_wikipedia download/
module trimmed to a single wiki — enwiki's much-larger multi-part dump is
out of scope.
"""
...
def fetch_sha1sums() -> dict[str, str]:
    """Fetch the simplewiki sha1sums manifest, filtered to the multistream files.
...
def main(argv: list[str] | None = None) -> int:
    ...
    for filename, sha1 in manifest.items():
        ...
        if not m:
            ...
            failed = True
            continue
        # Wikimedia keeps both /latest/ and the dated subdirectory live; using
        # the dated path means a slow download won't hit a "new latest" race
        # mid-transfer.
