...
python scripts/arxiv/arxiv_download.py              # HTML bodies (PDF fallback when no HTML) for papers
...
python scripts/enwiki/enwiki_index_rag.py           # data/enwiki/enwiki_rag.db (--limit 100 default; full ~19M)
...
This repo is the **backend only** (API + data + ingest scripts), served read-only over Tailscale; it serves no UI. Everything runs on **pop-os only** — the `scripts/` downloaders/indexers, the `data/<source>/*.db` + `/datasets/arxiv/arxiv.db` files they produce, the API that reads them, and the live `rag/` embed/download write paths. Run all `scripts/` on pop-os, then restart uvicorn there.
...
- `/arxiv/papers`, `/{id:path}`, `/{id:path}/content`, `POST /{id:path}/download`, `POST /{id:path}/embed`, `/arxiv/chunks`, `/arxiv/categories` — served from the monolithic `arxiv.db` (`api/db.py` `arxiv()`); plain SQL. `sort=relevance` is `bm25(papers_fts)`. `/categories` is the `{code: description}` taxonomy read (cached) from `data/arxiv/categories.csv` (used to label category codes; 503 if missing). `POST .../download` fetches one paper's LaTeXML HTML on demand (`rag.arxiv_fetch.fetch_paper_html`) and writes `html_content`; a 404 records `download_status='no_html'` (200). Needs `DATASETS_EMAIL`.
...
- `/enwiki/articles` (`?q=`, `?title=`, `?namespace=`), `/{page_id}`, `/{page_id}/content`, `POST /{page_id}/embed`, `/enwiki/resolve`, `/enwiki/chunks`, `/enwiki/doc-chunks` — local `enwiki.db` (`api/db.py` `enwiki()`); same shape as simplewiki (HTML `/content`, `/resolve`). `?q=` is FTS5 **trigram** over title + body. RAG via the `enwiki_index_rag.py` batch indexer or on-demand embed.
...
- **arxiv** — monolithic ~80 GB DB at `/datasets/arxiv/arxiv.db` (outside the repo; `/home` near-full), read-only via `api/db.py` `arxiv()`, no sharding; scripts default to `--db data/arxiv/arxiv.db` so pass `--db /datasets/arxiv/arxiv.db`. `arxiv_ingest.py` (OAI-PMH, 3 s/req; `--from`/`--until`/`--from-cache`/`--reset`), `arxiv_download.py` (HTML bodies; on a 404 falls back to the PDF → saves the raw file under `--bodies-dir` (default `<db parent>/bodies/`, e.g. `/datasets/arxiv/bodies/`) and stores extracted text in `papers.pdf_text` with `download_status='downloaded_pdf'`; `'no_body'` when arXiv has neither; auto-adds `pdf_text` column on older DBs; `--limit`/`--force`/`--retry-no-body` (re-run only the bodyless `'no_html'`/`'no_body'` rows, leaving downloaded papers untouched)), `arxiv_normalize_authors.py` (pre-Phase-3 backfill, idempotent), `arxiv_index_rag.py` (global, not sharded; full HTML else abstract-only; 1500/1800/150).
...
- **enwiki** (local DB) — full ~263 GB `enwiki.db` (~19M rows) served directly read-only via `api/db.py` `enwiki()` (old raspberrypi6 proxy gone). `articles_fts` is a **trigram** index over title + body, prebuilt inside the file — no parse/FTS script here, so a missing table means restore the DB. `enwiki_index_rag.py` (`--limit` 100 default, full ~19M ≈ years on local Ollama; same pipeline as simplewiki; `ENWIKI` profile == SIMPLEWIKI; `enwiki_rag_extract.py` entry point) is the batch RAG indexer; on-demand embed also available via `POST /enwiki/articles/{page_id}/embed`. `enwiki_rag.db` ships empty. `scripts/enwiki/enwiki_remote_server.py` is the old pi service, reference only.
