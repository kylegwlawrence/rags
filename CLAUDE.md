...
- `/arxiv/papers`, `/{id:path}`, `/{id:path}/content`, `POST /{id:path}/download`, `POST /{id:path}/embed`, `/arxiv/chunks`, `/arxiv/categories` — served from the single monolithic `arxiv.db` (`api/db.py` `arxiv()`); plain SQL, no fan-out. `/categories` returns the `{code: description}` taxonomy read (cached) from `data/arxiv/categories.csv` — the frontend fetches it once to label the Category/Categories fields in a paper's metadata pane (503 if the CSV is missing). `sort=relevance` is `bm25(papers_fts)`; date sorts exact. `POST .../download` fetches one paper's LaTeXML HTML from arXiv on demand (shared `rag.arxiv_fetch.fetch_paper_html`, the same fetch `arxiv_download.py` uses in bulk) and writes it onto `html_content`; a 404 records `download_status='no_html'` and returns that status with a 200 (the frontend shows a "no HTML available" note). Needs `DATASETS_EMAIL`. `/chunks` is the global `arxiv_rag.db`.
- `/openalex/works` (`?q=`, `?year=`, `?cited_by_min/max=`, `?venue=`, `?domain=`, `?field=`, `?author=`, `?embedded=`, `?sort=`), `/{short_id}`, `POST /{short_id}/download`, `/openalex/chunks` — `?domain=`/`?field=` exact-match the work's primary-topic hierarchy. `POST .../download` fetches one work's open-access PDF (`pdf_url`/`oa_url`) on demand via the shared `rag.openalex_fetch.fetch_work_pdf` (the same fetch `openalex_fetch_bodies.py` uses in bulk) and saves it under `data/openalex/bodies/{short_id}.pdf` for the `pdfs` pipeline — OpenAlex has no body text, so unlike arxiv/sec it writes nothing back into openalex.db (only a `body_status` row); returns `status='no_pdf'` (200) when no accessible OA PDF exists. Needs `DATASETS_EMAIL`.
- `/factbook/countries`, `/{id}`, `/factbook/chunks`
- `/gutenberg/texts`, `/{id}`, `/{id}/content`, `/gutenberg/chunks`
- `/simplewiki/articles` (`?q=`, `?title=`, `?namespace=`, `?category=`, `?embedded=`), `/{page_id}`, `/{page_id}/content`, `POST /{page_id}/embed`, `/simplewiki/resolve`, `/simplewiki/chunks` + `/simplewiki/categories` — `/{page_id}/content` renders the wikitext to **HTML** via `rag.wiki_render` (KaTeX-ready math, infoboxes, tables, refs; frontend `contentType: 'html'`). `/resolve?title=` maps an exact `[[wikilink]]` title to its article (index-backed `idx_articles_title`, first-letter-capitalised fallback; 404 if none) — powers in-app wikilink navigation. `?category=` is a case-insensitive substring match on category name (normalized like the stored names for underscores/whitespace, so e.g. `actor` matches `American movie actors`); detail `/{page_id}` carries the article's `categories`. `/categories` lists distinct categories with article counts (`?q=` name substring, `?sort=count|name`). Both back onto the `page_categories` table from `simplewiki_index_categories.py` (503 if not built).
- `/enwiki/articles` (`?q=`, `?title=`, `?namespace=`), `/{page_id}`, `/{page_id}/content`, `POST /{page_id}/embed`, `/enwiki/resolve`, `/enwiki/chunks` + `/enwiki/doc-chunks` — served from the local `enwiki.db` (`api/db.py` `enwiki()`); plain SQL, same shape as simplewiki — incl. HTML-rendered `/{page_id}/content` (`rag.wiki_render`) and `/resolve?title=` wikilink resolution (the index hint matters here: namespace 0 is ~all 19M rows, so the planner would otherwise scan). `?q=` is FTS5 (trigram) over title **and** body. Embedding (and so `/chunks`) is on-demand only (no batch indexer).
...
Write paths are the on-demand `/embed` routes (arxiv, openalex, gutenberg, simplewiki, ecfr, eurlex, enwiki, pdfs, openstax, federal_register, github_readmes, sec_edgar → their `<source>_rag.db`) plus `POST /arxiv/papers/.../download` (writes `html_content` onto `arxiv.db`), `POST /sec_edgar/.../download` (writes a body onto `sec_edgar.db`), and `POST /openalex/works/.../download` (writes a `body_status` row onto `openalex.db`; the PDF itself lands on disk under `data/openalex/bodies/`). Everything else is read-only.
...
- `openalex_fetch_bodies.py` — **Standalone** body fetcher (mirrors `loc_fetch_bodies.py` / `sec_edgar_fetch_bodies.py`): walks the open-access subset (`is_oa=1`, highest-cited first) and downloads each work's actual PDF from `pdf_url` (falling back to `oa_url`) into `data/openalex/bodies/{short_id}.pdf` — OpenAlex serves only the OA *pointers*, never the body. It only downloads files; it builds no index. The PDFs flow into the self-contained `pdfs` scripts (e.g. `pdfs_ingest.py --db data/openalex/openalex_pdfs.db --incoming data/openalex/bodies`). Streams to a `.part` temp file and only keeps responses that start with the `%PDF-` magic, so landing pages / paywalls never leave a poisoned file. Resumable: per-work outcomes recorded in a `body_status` table (`fetched`/`no_pdf`/`error`), so re-runs skip done/known-no-PDF works; transient `error` rows retry unless `--skip-errors`. Needs `DATASETS_EMAIL`. The fetch + `body_status` logic lives in `rag/openalex_fetch.py`; the API's `POST /openalex/works/.../download` route reuses it in-process for the single-work "Download PDF" button. Flags: `--db`, `--limit` (default 50), `--out-dir` (default `data/openalex/bodies`), `--delay` (default 3 s), `--skip-errors`.
...
- `rag/` — `chunker.py` (`chunk_doc` / `chunk_markdown`), `cleaner.py` (`CLEANER_VERSION`), `embedder.py` (nomic-embed-text:v1.5 768d, `OLLAMA_URL`), `render.py` (arxiv HTML→md), `wikitext.py` (wikitext→md, for embedding), `wiki_render/` (wikitext→**HTML** for the simplewiki/enwiki Content view — ported from the local_wikipedia `render/` package; KaTeX math + `\ce{}` chemistry, infoboxes, hatnotes, reflists, wikitables, poems, indicators; entry point `convert_wikitext_to_html`), `sec_filing.py` (SEC submission fetch + primary-document extraction into clean text *and* render HTML via `extract_primary`, shared by the fetch-bodies script and the API download route), `arxiv_fetch.py` (`fetch_paper_html` — arxiv LaTeXML HTML fetch, shared by the bulk downloader + API download route), `openalex_fetch.py` (`fetch_work_pdf` — walks a work's OA `pdf_url`/`oa_url`, saves the PDF to disk, + the `body_status` schema; shared by the fetch-bodies script and the API download route), `retriever.py` (RRF), `retry.py`, `schema.py`, `indexer.py`.
