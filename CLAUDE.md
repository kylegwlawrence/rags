...
- `/openstax/books` (`?q=`, `?subject=`), `/{book_id}`; `/openstax/sections` (`?q=`, `?book_id=`, `?subject=`, `?embedded=`, `?sort=`), `/sections/{section_id}`, `/sections/{section_id}/content`, `POST /sections/{section_id}/embed`, `/openstax/chunks` (`?q=`, `?subject=`, `?book_id=`, `?chapter_number=`) + `/openstax/doc-chunks` — OpenStax textbooks as `books` / `chapters` / `sections`. `book_id` is the collection slug (e.g. `calculus-volume-1`); a section's id is `{book_id}/{module_id}` (slash-containing, addressed via a `{section_id:path}` route like pydocs' `doc_path`). The browsable unit is the **section**: `?q=` is FTS5 over section title + learning objectives + body, `sort=relevance` requires `q` (else document/reading order). `/content` returns the section body as **light Markdown** (`##` sub-section headings, `-` lists) with **inline `\(…\)` / display `\[…\]` LaTeX** (formulas are presentation MathML in the source CNXML, rebuilt to LaTeX at download time — see the openstax script notes); the frontend (`contentType: 'markdown'`) renders the Markdown and typesets the math with KaTeX (vendored under `frontend/vendor/katex/`, loaded with `marked` in `index.html`). RAG is on-demand or batch: each chunk's `section` is its "Chapter — Section" label; `?embedded=` filters by chunk presence. `/openstax/chunks` accepts optional metadata filters that scope the semantic search to a slice of the corpus: `?subject=` and `?book_id=` are **repeatable** (OR within a list — pass several `book_id`s to pull a course plus its prerequisite books in one call), `?chapter_number=` is single-value (only meaningful with one book); filters AND across types, an unknown value just yields an empty result. They resolve to an allowlist of `section_id`s from `openstax.db` that constrains retrieval (sparse arm filtered exactly in SQL, dense arm over-fetched then filtered — see `rag/retriever.py` `allowed_doc_ids`). Each chunk hit also carries `book_id` / `subject` / `chapter_number` / `chapter_title` provenance, so a consumer can read the dominant-subject distribution off an unfiltered query as a lightweight routing signal.
...
- `/federal_register/documents` (`?q=`, `?type=`, `?agencies=`, `?publication_year=`, `?embedded=`, `?sort=`), `/{document_number}`, `/{document_number}/content`, `POST /{document_number}/embed`, `/federal_register/chunks` + `/federal_register/doc-chunks` — one row per Federal Register document (1994–present); `document_number` is the id. `?q=` is FTS5 over title + abstract; `?type=` is an exact match on document type (`Rule`, `Proposed Rule`, `Notice`), `?agencies=` a substring match; `sort=relevance` requires `q` (else `newest`/`oldest`). `/content` returns the abstract (falling back to excerpts) as plain text. RAG is on-demand or batch; `?embedded=` filters by chunk presence.
- `/geonames/places` (`?q=`, `?country_code=`, `?feature_class=` (repeatable), `?feature_code=` (repeatable), `?min_population=`), `/{geonameid}`, plus `/geonames/feature_classes` and `/geonames/feature_codes` (`?feature_class=` repeatable) lookup endpoints that populate the frontend's multi-select dropdowns — one row per named geographic feature (~13M; `geonameid` is the id). `?q=` is FTS5 over name + country_name + feature_description; default sort is population-descending (most notable first), `q` sorts by relevance. **No RAG / `/chunks`** — rows are one-line records, not documents (each row carries a synthetic `sentence` summary instead). The places table is huge, so always pass at least one filter; an open query scans all 13M rows.
- `/github/readmes` (`?q=`, `?owner=`, `?source_list=`, `?embedded=`, `?sort=`), `/{repo:path}`, `/{repo:path}/content`, `POST /{repo:path}/embed`, `/github/chunks` + `/github/doc-chunks` — one row per repository README harvested from 15 curated "awesome" lists; `repo` is the `owner/name` slug (slash-containing, addressed via a `{repo:path}` route). Only `status='fetched'` rows are served. `?q=` is FTS5 over repo name + README body, `?owner=` a substring match, `?source_list=` the awesome-list it was discovered from. `/content` returns the raw README markdown. RAG is on-demand or batch (batch indexer defaults to `--limit 100`); `?embedded=` filters by chunk presence. Low-quality READMEs (link-dumps, too-short, image-only, non-English) are dropped by `github_readmes_prune.py`, which also syncs the deletions into the RAG DB.

The `/sec_edgar/filings` list (and detail) now surfaces metadata-only filings whose body hasn't been downloaded — `?downloaded=` narrows to fetched/unfetched. The API's write paths are the on-demand `/embed` routes — now on most sources (arxiv, openalex, gutenberg, simplewiki, ecfr, eurlex, enwiki, pdfs, openstax, federal_register, github_readmes, sec_edgar → their `<source>_rag.db`) — plus `POST /sec_edgar/.../download` (writes a fetched body onto `sec_edgar.db`). Everything else is read-only.
