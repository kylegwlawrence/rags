...
This repo is the **backend only** (API + data + ingest scripts). The frontend has been split into its own repo, **`datasets_frontend`**, which lives and runs on **raspberrypi6** — a small FastAPI host (`server.py` + a `static/` directory) that serves the static single-page UI under `/ui/` and reverse-proxies every other path to the pop-os backend (`DATASETS_BACKEND_URL`, default pop-os Tailscale IP). It binds the Pi's Tailscale IP on port 8002, so `http://raspberrypi6:8002/` is the single user-facing URL (`/` → 302 `/ui/`). Edit the UI in that repo; its own README covers the systemd deployment (`datasets-frontend.service`).

This backend **no longer serves the UI** — there is no `/ui` mount in `api/main.py`. Deployment splits across two Tailscale machines: **pop-os** runs the backend API (`uvicorn api.main:app`, port 8002), **raspberrypi6** runs the `datasets_frontend` host.

**Where scripts and data live:** this backend repo runs on **pop-os only** — that's where the `scripts/` downloaders and indexers run, where the `data/<source>/*.db` files and `/datasets/arxiv/arxiv.db` they produce live, and where the backend API reads them (plus the live `rag/` embed/download write paths). **raspberrypi6 runs the separate `datasets_frontend` proxy only** — it has no copy of this repo's data or `/datasets` mount. Run all `scripts/` on pop-os, then restart uvicorn on pop-os. `rag/` is a shared library (not a process): it executes on whichever machine imports it — the scripts and the API, both on pop-os.
...
- **openstax** — `openstax_download.py`: shallow+sparse `git clone` of `osbooks-*` repos (`collections/`+`modules/`+`media/`) on root FS → `books`/`chapters`/`sections`, deletes clone; bodies light Markdown (`##`/`###` + inline `\(…\)`/`\[…\]` LaTeX rebuilt from MathML by stdlib `rag/mathml.py`); ships all English shelves (70 books / 10 subjects / ~9.4k sections); `--repos`/`--work-dir`/`--keep-clones`/`--skip-images`; **editing `rag/mathml.py` → re-run download, then FTS, then re-embed** (LaTeX baked in at download). Images are copied to `data/openstax/media/{repo}/` (namespaced by repo since bundle volumes share one `media/`) and referenced in the body as Markdown image links `![alt](/openstax/media/{repo}/file.jpg)`, served by the API's `/openstax/media` static mount (`api/main.py`); `--skip-images` reverts to text-only bodies. `openstax_index_rag.py` section-aware `chunk_markdown`, doc leads with title + objectives then body, chunk `section` = "Chapter — Section"; `build_doc` + parsers in `rag/openstax.py` shared with live-embed; `openstax_rag_extract.py` entry point.
...
- **justice_canada** — consolidated Canadian acts + regulations from the Justice Canada `laws-lois-xml` corpus. `download.py` (fetches the XML corpus → `data/justice_canada/laws-lois-xml/`; `--language` en/fr/both), `parse.py` (XML → `justice_canada.db`; `acts`/`regulations` tables, body as Markdown via `rag/justice_canada.py` `body_to_markdown`; `INSERT OR REPLACE` idempotent; `--language`/`--type`/`--corpus-dir`). `index_fts.py` builds **two** external-content FTS5 tables — `acts_fts` (short_title + long_title + running_head + body) and `regulations_fts` (short_title + long_title + enabling_authority + body); both required for `?q=`/`sort=relevance`, which the router joins per-side and merges by `bm25`. No RAG indexer (no chunks).
