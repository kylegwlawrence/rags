...
**Where scripts and data live:** the repo is checked out on both machines (each with its own `.venv`), but the ingest pipeline runs on **pop-os only** — that's where the `scripts/` downloaders and indexers run, where the `data/<source>/*.db` files and `/datasets/arxiv/arxiv.db` they produce live, and where the backend API reads them (plus the live `rag/` embed/download write paths). **raspberrypi6 runs the frontend proxy only** — its `data/` tree is empty and it has no `/datasets` mount, so running an ingest script there would write into a tree the API never reads. Run all `scripts/` on pop-os, then restart uvicorn on pop-os. `rag/` is a shared library (not a process): it executes on whichever machine imports it — the scripts and the API, both on pop-os.
