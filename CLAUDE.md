...
- `/simplewiki/articles` (`?q=`, `?title=`, `?namespace=`, `?category=`, `?embedded=`), `/{page_id}`, `/{page_id}/content`, `POST /{page_id}/embed`, `/simplewiki/chunks` + `/simplewiki/categories` — `?category=` is a case-insensitive substring match on category name (normalized like the stored names for underscores/whitespace, so e.g. `actor` matches `American movie actors`); detail `/{page_id}` carries the article's `categories`. `/categories` lists distinct categories with article counts (`?q=` name substring, `?sort=count|name`). Both back onto the `page_categories` table from `simplewiki_index_categories.py` (503 if not built).
