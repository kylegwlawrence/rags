...
- `api/main.py` — FastAPI app, mounts the five routers, exposes `/health`.
...
- `api/routers/{arxiv,factbook,openalex,gutenberg,simplewiki}.py` — one router per datasource. SQL is inline; the routers are intentionally thin.
- `api/_chunks.py` — shared `/chunks` route factory used by all five routers. Wraps `rag.retriever.retrieve` with the empty-`q` → 400, missing-rag.db → 503, Ollama-down → `used_dense=False` dispatch.
- `api/_fts.py` — `translate_fts_errors` context manager that wraps the inline FTS5-joined queries in the arxiv/openalex/simplewiki list endpoints; maps missing-table → 503 (naming the indexer script), bad FTS5 syntax → 400.
